...
Return conference papers first, then `arXiv-only preprints`. For each result, include title, authors, year, venue/status, primary link, and brief relevance. Label uncertain venue or review status; never present arXiv-only work as peer-reviewed.
